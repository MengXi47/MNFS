...
template <>
struct formatter<mnfs::Status> : formatter<mnfs::status_code_t> {
  template <typename FormatContext>
  auto format(const mnfs::Status &status, FormatContext &ctx) const {
    auto msg = status.message();
    if (msg.empty()) {
      return fmt::format_to(ctx.out(), "{}({})", mnfs::StatusCode::toString(status.code()), status.code());
    }
    return fmt::format_to(ctx.out(), "{}({}) {}", mnfs::StatusCode::toString(status.code()), status.code(), msg);
  }
};
