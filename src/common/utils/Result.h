...
#define RETURN_ERROR(result) return mnfs::makeError(std::move(result.error()))

#define MAKE_ERROR_F(code, ...) mnfs::makeError((code), fmt::format(__VA_ARGS__))
...
#define CO_RETURN_ERROR(result)                                                                     \
  do {                                                                                              \
    ...
    if (result.error().message().empty())                                                           \
      co_return mnfs::makeError(result.error().code(),                                              \
                                fmt::format("{}:{}, '{}' has error", __FILE__, __LINE__, #result)); \
    else                                                                                            \
      co_return mnfs::makeError(std::move(result.error()));                                         \
  } while (0)
...
template <typename T>
struct formatter<mnfs::Result<T>> : formatter<T> {
  template <typename FormatContext>
  auto format(const mnfs::Result<T> &result, FormatContext &ctx) const {
...
template <>
struct formatter<folly::Unexpected<mnfs::Status>> : formatter<mnfs::Status> {
  template <typename FormatContext>
  auto format(const folly::Unexpected<mnfs::Status> &status, FormatContext &ctx) const {
    return formatter<mnfs::Status>::format(status.error(), ctx);
  }
};
